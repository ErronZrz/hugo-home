...
如果访问密码正确，API key 也有效，那么请开始尽情享用专属于你的 ChatGPT 助手吧。使用 OpenAI API 方式来提问的不足在于，API key 会过期，并且回答的质量上会稍逊于直接来自 ChatGPT 官网的回答，但优势在于酷炫的外观和支持自定义参数。最重要的一点是，自己搭的网站能做到全天候秒回，有了它之后，你再也不必忍受官网的 `Access Denied`、漫长的「思考时间」以及各种用着用着突然跳出来的网络错误提示:sweat_smile:。
