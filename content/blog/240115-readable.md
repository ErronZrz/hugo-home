...
对于 999,949,999,999,999,999，当初正是把它当成 999,949,000,000,000,000 这样一个不精确的值，才把问题解决的。这里，不精确的值**小于**原值，所以除以 $base_\text{PB}$ 后四舍五入才能够达到想要的**不进位**的效果。
...
难道我们真的要屈服于一堆 `xxx.x5` 吗？明明只需要把小数点后的那一位关键数字求出来就好。
...
有没有一种可能，此时一种暴力的方式是手动求出 0.0, 0.1, 0.2……这十种结果相互之间的边界，再在代码里依次拿它们和 `num` 比较？暴力法虽然可耻，但有用，而且是非常有用。

众所周知，$(0.95)_{10}=(0.11\overline{1100})_2$，其他的 `0.x5` 也照样可以轻松写出：

$(0.05)_{10}=(0.00\overline{0011})_2$, $(0.15)_{10}=(0.00\overline{1001})_2$, $(0.25)_{10}=(0.01)_2$, $(0.35)_{10}=(0.01\overline{0110})_2$, $(0.45)_{10}=(0.01\overline{1100})_2$, $(0.55)_{10}=(0.10\overline{0011})_2$, $(0.65)_{10}=(0.10\overline{1001})_2$, $(0.75)_{10}=(0.11)_2$, $(0.85)_{10}=(0.11\overline{0110})_2$, $(0.95)_{10}=(0.11\overline{1100})_2$。

这样一来，我们就能够计算 $base_\text{EiB}$ 乘以各种 `0.x5` 的精准边界值，例如乘以 0.05 再向上取整就等于 `0xCC_CCCC_CCCC_CCCD`。而且这个数不仅适用于最终单位是 EiB 的情况，结合之前找到的规律，它右移 10 位就等于 $\left\lceil0.05\cdot base_\text{PiB}\right\rceil-1$，以此类推。
...
这里的 `sort.SearchInts` 可以在有序整数切片中利用二分法寻找一个值 `v` 的插入索引，准确地说，它会返回满足 `v <= arr[i]` 的最大索引 `i`，如果不存在这样的索引，则返回切片长度，也就是告诉调用者「可以直接把 `v` 接在最后」。

在上面的代码中，`val` 的值为 0-9 分别代表整数部分不变而四舍五入后小数点后 1 位的值应该等于 `val`；`val` 的值为 10 则代表当前的数已经达到了 `xxx.95`，因此需要进位。
...
在这种浩荡之中，又淡淡地散发着那二进制所独有的、不可端倪的、诡谲的美感。
...
但没有人能定义这个问题的「结束」。也许还有错误的测试用例伺机待发，也许还会出现更好的方法，也许哪一天连 AI 都能够一次性把这个问题分析透彻（~~也许那个时候人类对地球的统治都已经结束了~~）……

总之，感谢你能读到这里。

最后还需要鸣谢 Windows 计算器对本文的大力支持，没有它纵享丝滑的进制转换以及超大整数运算，我估计没有机会意识到十进制小数和二进制小数之间美妙的联系。
