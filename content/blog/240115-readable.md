...
我们把一个有理数集合中相邻两个数之间的差称为「刻度距离」，例如一把直尺的刻度距离一般是 1mm，64 位无符号整数的刻度距离是 1。
...
然而很遗憾，此时的 $r$（也就是代码中的 `remainder`）只是小于 $2^{60}$，但不保证小于 $2^{53}$，因此在向 `float64` 转换的路上，最末尾的有效数字是无法逃离被吞噬的命运的。
...
等等。「小数点后的**那一位**关键数字」？那</u>**一位**</u>，关键数字？

</u>**一位**</u>数字有几种可能性？无非是从 0 到 9 而已。
...
但我的能力不足以定义这个问题的「**结束**」。也许被忽略的错误测试用例正伺机待发，也许还会出现更好的方法，也许哪一天连 AI 都能够在一秒钟之内把这个问题分析透彻（~~也许那个时候人类对地球的统治都已经结束了~~）……

总之，感谢你能读到这里。

最后还需要鸣谢 Windows 计算器对本文的大力支持，如果离开了它纵享丝滑的进制转换以及超大整数运算，我甚至没有机会意识到十进制小数和二进制小数之间美妙的联系。
